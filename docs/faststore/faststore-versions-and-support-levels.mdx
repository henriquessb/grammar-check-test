...
One of main changes from FastStore v1 to FastStore v2+ is the separation between VTEX code and store-specific customizations. This separation introduced the [`core`](https://developers.vtex.com/docs/guides/faststore/project-structure-overview#packagejson) package for VTEX code and reserved the `/src` folder for store-specific customizations. Below are the key differences betwen the version:
